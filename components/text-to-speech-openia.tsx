...
export default function TextToSpeech() {
  ...
  // Limpiar audioUrl y pausar audio cuando el texto cambie
  useEffect(() => {
    setAudioUrl(null)
    setIsPlaying(false)
    if (audioRef.current) {
  ...
  const fetchAudio = async (t) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        throw new Error(err.error || "Error generating audio")
      }
  ...
  const handlePlay = async () => {
    if (!audioUrl) {
      ...
      setTimeout(() => {
        ...
        setIsPlaying(true)
      }, 100) // Espera a que el audio esté listo
    } else {
      audioRef.current?.play()
      setIsPlaying(true)
    }
          ...
          <CardContent className="space-y-6">
            ...
            <div className="bg-gray-50 rounded-lg p-4">
              <div className="flex items-center justify-between mb-4">
                <div className="flex items-center gap-3">
                  ...
                  >
                    ...
                    {isPlaying ? "Pausar" : loading ? "Generando..." : "Reproducir"}
                  </Button>

                  <Button onClick={handleStop} variant="outline" disabled={loading}>
                    <RotateCcw className="h-4 w-4 mr-2" />
                    Reiniciar
                  </Button>

                  <Button onClick={handleSpeedChange} variant="outline" disabled={loading}>
                    {playbackRate}x
                  </Button>
                </div>
              ...
              <div className="flex justify-between text-xs text-gray-500 mt-1">
                <span>{audioRef.current ? audioRef.current.currentTime.toFixed(1) : "0:00"}</span>
                <span>{audioRef.current && audioRef.current.duration ? audioRef.current.duration.toFixed(1) : "0:00"}</span>
              </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => {
                        ...
                        if (audioRef.current) audioRef.current.playbackRate = Number.parseFloat(e.target.value)
                      }}
